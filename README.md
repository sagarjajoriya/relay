...
- **M6 — Search**: done. Postgres full-text search over messages: stored generated `tsvector` column + GIN index, `websearch_to_tsquery` parsing (phrases, `-exclusion`, never errors on user input), `ts_rank` ordering, `ts_headline` snippets with control-char match delimiters (XSS-safe rendering without HTML parsing). Channel visibility enforced inside the SQL — private content is unfindable, not post-filtered. `GET /workspaces/:id/search` + search box in the web UI.
- **M7 — Attachments**: done. Pre-signed direct-to-storage uploads (S3-compatible; MinIO locally) — file bytes never transit the API. `POST /channels/:id/attachments` issues a PENDING row + signed PUT URL; attachment IDs ride on message send, where the API HEAD-verifies the real object, enforces uploader/channel/size, and links PENDING→ATTACHED atomically (doubles as the double-link guard). Reads return short-lived signed GET URLs; orphaned uploads are reaped by a delayed BullMQ job.
- **M8 — Threads, reactions, mentions, unread**: done. Single-level threads (`parentId` + denormalized `replyCount`, replies excluded from the timeline, thread panel UI), aggregated reactions (`{emoji, count, userIds}` so shared broadcasts stay viewer-agnostic), canonical `<@userId>` mention tokens with composer autocomplete + MENTION notifications, per-channel unread watermarks (`ChannelRead`) with live badge bumps via a workspace-room `channel.activity` event, thread-reply notifications targeting participants only, and notifications mark-all-read.
